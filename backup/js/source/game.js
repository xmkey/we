!function($){
	...
	var gameContainer=$("#game-container");
 	function gameline(container,line){
 		...
 		this.canvas2.width=$(container).width();
 		this.canvas2.height=$(container).height();
 	...
 	gameline.prototype={
 		...
 		_getPoints:function(){
 			...
 			for(var i=0;i<line.length;i++){
 				...
 				if(!points[x1+","+y1]){
 					...
 					var item=$("<div class='point'></div").appendTo(this.container);
 					item.data("x",x1);
 					item.data("y",y1);
 					item.css({
 						left:x1-item.width()/2,
 						top:y1-item.height()/2
 					})
 					points[x1+","+y1]=item;
 				}
 				if(!points[x2+","+y2]){
 					...
 					item.data("y",y2);
 					item.css({
 						left:x2-item.width()/2,
 						top:y2-item.height()/2
 					})
 					points[x2+","+y2]=item;
 				}
 		...
 		_bind:function(){
 			...
 			$(document).on("touchstart",function(e){
 				...
 				// if($(e.target).parent()==self.container)
 		...
 		_onmove:function(e,self){
 			if(self.lineArr.length){
				...
				ctx.beginPath();
	 			...
	 			ctx.lineTo(touchX-self.offset.left,touchY-self.offset.top);
